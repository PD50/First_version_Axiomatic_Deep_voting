"""
Enhanced Set Transformer v2 for voting rule synthesis.

Architectural upgrades over v1:
1. Dual-pathway input encoding (one-hot + pairwise comparison features)
2. ISAB (Induced Set Attention Blocks) for O(nm) complexity with 55 voters
3. Multi-seed PMA decoder (m seeds, one per alternative)
4. GELU activation in feedforward layers
...
- Hornischer & Terzopoulou (2025) "Learning How to Vote with Principles" — axiom framework
...
class MAB(nn.Module):
    """
    Multihead Attention Block with pre-layer-norm and GELU activation.
...
class ISAB(nn.Module):
    """
    Induced Set Attention Block from Lee et al. (2019).

    Uses m_ind inducing points to reduce complexity from O(n^2) to O(n*m_ind).
    Critical for scaling to 55 voters.

    ISAB_m(X) = MAB(X, H) where H = MAB(I, X)
    I = learned inducing points of shape (m_ind, d_model)
    """
...
class PMA(nn.Module):
    """
    Pooling by Multihead Attention with pre-FF.
    PMA_k(Z) = MAB(S, FF(Z)), S = learned seeds.
    """
...
# ============================================================
# Enhanced Set Transformer v2
# ============================================================

class SetTransformerV2(nn.Module):
    """
    Enhanced Set Transformer with dual-pathway input and ISAB encoder.

    Key differences from v1:
    1. Dual input: one-hot ranking + pairwise comparison matrix
    2. Mixed encoder: SAB layers + ISAB layers for efficiency at scale
    3. Multi-seed PMA: m seeds (one per alternative) for richer decoding
    4. GELU activations throughout
    5. Residual output head

    Architecture:
        [one-hot → proj_A] + [pairwise → proj_B] → fused embedding
        → SAB → ISAB → ISAB → SAB (mixed encoder)
        → PMA(m seeds) → SAB (decoder)
        → per-alternative output head with residual
    """
    ...
    def __init__(
        ...
        # For each voter: C[i,j] = 1 if voter prefers alt i over alt j
        ...
        # === Mixed Encoder: SAB + ISAB ===
        # First layer: full self-attention to capture all pairwise voter interactions
        # Middle layers: ISAB for efficiency (O(n * m_ind) instead of O(n^2))
    ...
    def forward(self, x, x_pairwise=None):
        ...
        h_res = h + self.output_ff(h_norm)     # residual
...
def profile_to_set_input_v2(profile, max_num_voters, max_num_alternatives):
    """
...
def SetTransformerV2_2logits(model, X):
    """
...
def SetTransformerV2_2rule_prediction(model, profile, full=False):
    """Predict winners from a single profile."""
    model.eval()
    with torch.no_grad():
        t_oh, t_pw = profile_to_set_input_v2(
...
def SetTransformerV2_2rule(model, full=False):
    """Returns a voting rule function from the model."""
    return lambda profile: SetTransformerV2_2rule_prediction(model, profile, full)


def SetTransformerV2_2rule_prediction_n(model, profile, num_samples, full=False):
    """
...
def SetTransformerV2_2rule_n(model, num_samples, full=False):
    """Returns a neutrality-averaged voting rule function."""
    return lambda profile: SetTransformerV2_2rule_prediction_n(
